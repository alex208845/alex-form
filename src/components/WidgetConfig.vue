  ...
  <div v-if="show">
    <el-form label-position="top">
      ...
      <el-form-item :label="$t('fm.config.widget.size')"
        ...
        {{$t('fm.config.widget.width')}} <el-input style="width: 90px;"
                  type="number"
                  v-model.number="data.options.size.width"></el-input>
        {{$t('fm.config.widget.height')}} <el-input style="width: 90px;"
                  type="number"
      ...
      <el-form-item :label="$t('fm.config.widget.option')"
        ...
        <el-radio-group v-model="data.options.remote"
          ...
          <el-radio-button :label="false">{{'数据获取'}}</el-radio-button>
        ...
        <template v-else>
          ...
          <div style="margin-left: 22px;">
            <el-button type="text"
                       @click="handleAddOption">{{$t('fm.actions.addOption')}}</el-button>
            <el-button type="text"
                       @click="getDatasFromServe">{{'远端数据'}}</el-button>
          </div>
        </template>

      </el-form-item>

      <el-form-item :label="$t('fm.config.widget.remoteData')"
      ...
      <template v-if="data.type=='imgupload'">
        ...
        <template v-else>
          <el-form-item :label="$t('fm.config.widget.imageAction')"
                        :required="true">
        ...
        <el-form-item :label="$t('fm.config.widget.columnOption')">
          <draggable tag="ul"
            ...
            <li v-for="(item, index) in data.columns"
              ...
              <i class="drag-item"
                 style="font-size: 16px;margin: 0 5px;cursor: move;"><i class="iconfont icon-icon_bars"></i></i>
              <el-input :placeholder="$t('fm.config.widget.span')"
                        size="mini"
                        style="width: 100px;"
                        type="number"
          ...
          <div style="margin-left: 22px;">
            <el-button type="text"
                       @click="handleAddColumn">{{$t('fm.actions.addColumn')}}</el-button>
          </div>
        </el-form-item>
      ...
      <template v-if="data.type != 'grid'">
        <el-form-item :label="$t('fm.config.widget.attribute')">
          <el-checkbox v-model="data.options.readonly"
                       v-if="Object.keys(data.options).indexOf('readonly')>=0">{{$t('fm.config.widget.readonly')}}</el-checkbox>
          <el-checkbox v-model="data.options.disabled"
                       v-if="Object.keys(data.options).indexOf('disabled')>=0">{{$t('fm.config.widget.disabled')}} </el-checkbox>
          <el-checkbox v-model="data.options.editable"
                       v-if="Object.keys(data.options).indexOf('editable')>=0">{{$t('fm.config.widget.editable')}}</el-checkbox>
          <el-checkbox v-model="data.options.clearable"
                       v-if="Object.keys(data.options).indexOf('clearable')>=0">{{$t('fm.config.widget.clearable')}} </el-checkbox>
          <el-checkbox v-model="data.options.arrowControl"
                       v-if="Object.keys(data.options).indexOf('arrowControl')>=0">{{$t('fm.config.widget.arrowControl')}}</el-checkbox>
          <el-checkbox v-model="data.options.isDelete"
                       v-if="Object.keys(data.options).indexOf('isDelete')>=0">{{$t('fm.config.widget.isDelete')}}</el-checkbox>
          <el-checkbox v-model="data.options.isEdit"
                       v-if="Object.keys(data.options).indexOf('isEdit')>=0">{{$t('fm.config.widget.isEdit')}}</el-checkbox>
    ...
    <el-dialog title="请选择选项"
               :visible.sync="dialogVisible"
               width="20%">
      <el-cascader :options="optionszzz"
...
export default {
  ...
  methods: {
    handleChange (value) {
      console.log(value);
      console.log(this.optionszzz)
      let first = {}
      this.optionszzz.forEach(item => {
        if (item.value === value[0]) {
          item.children.forEach(ite => {
            if (ite.value === value[1]) {
              this.selectOptions = ite.children
            }
          })
        }
      })
    ...
    addOptions () {
      if (this.data.options.showLabel) {
        this.data.options.options = this.selectOptions
      } else {
        this.data.options.options = this.selectOptions
      }
    },
    ...
    getDatasFromServe () {
      ...
      var token = 'e2552cf75e4f41778edbcae0427a2b92';
      var meid = "123"
      this.$axios.defaults.headers.common["token"] = token;
      this.$axios.defaults.headers.common["meid"] = meid;
      this.$axios.defaults.headers.common["Accept"] = "*/*";
      this.$axios.post('http://192.168.1.13:8088/siws/api/valuation/data/list', { valuationOptionDTO: {} }).then(function (res) {
        // console.log(res.data.data)

        if (res.data.code === 0) {
          res.data.data.forEach(item => {
            if (item.valuationCode && item.optionList.length === 0) {
              that.optionszzz.push(
                {
                  value: item.valuationCode,
                  label: item.valuationName,
                  children: []
                })
            } else if (item.valuationCode && item.optionList.length > 0) {
              let arr = { value: item.valuationCode, label: item.valuationName, children: [] }
              item.optionList.forEach(ite => {
                if (ite.itmsList.length === 0) {
                  arr.children.push({
                    value: ite.voCode,
                    label: ite.voName,
                  })
                } else {
                  let arrList = { value: ite.voCode, label: ite.voName, children: [] }
                  // debugger
                  if (ite.itmsList.length !== 0) {
                    ite.itmsList.forEach(it => {
                  ...
                  arr.children.push(arrList)
                }

              })
              that.optionszzz.push(arr)
            }
          })
          console.log(that.optionszzz)
        }
